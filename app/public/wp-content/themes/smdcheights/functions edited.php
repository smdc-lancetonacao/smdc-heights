...
function enqueue_custom_menu_assets() {
    ...
    wp_localize_script('custom-menu-script', 'projectSearchAjax', array(
        'ajax_url' => admin_url('admin-ajax.php'),
        'nonce' => wp_create_nonce('project_search_nonce')
    ));
}
...
function project_details_callback($post) {
    ...
    $property = get_post_meta($post->ID, 'project_property', true);
    $status = get_post_meta($post->ID, 'project_status', true);
    
    echo '<table class="form-table">';
    echo '<tr><th><label for="project_location">Location:</label></th>';
    echo '<td><input type="text" id="project_location" name="project_location" value="' . esc_attr($location) . '" placeholder="e.g., Bay Area, Pasay, Quezon" style="width: 100%;" /></td></tr>';
    
    echo '<tr><th><label for="project_property">Property:</label></th>';
    echo '<td><input type="text" id="project_property" name="project_property" value="' . esc_attr($property) . '" placeholder="e.g., Jade Residence, Ruby Residence" style="width: 100%;" /></td></tr>';
    
    echo '<tr><th><label for="project_status">Status:</label></th>';
    echo '<td><select id="project_status" name="project_status" style="width: 100%;">';
    echo '<option value="available"' . selected($status, 'available', false) . '>Available</option>';
    echo '<option value="sold_out"' . selected($status, 'sold_out', false) . '>Sold Out</option>';
    echo '<option value="coming_soon"' . selected($status, 'coming_soon', false) . '>Coming Soon</option>';
    echo '</select></td></tr>';
    echo '</table>';
}
...
function save_project_details($post_id) {
    ...
    if (isset($_POST['project_property'])) {
        update_post_meta($post_id, 'project_property', sanitize_text_field($_POST['project_property']));
    }
...
// Register custom taxonomy for locations (e.g., Bay Area, Makati City)
function register_project_taxonomies() {
    register_taxonomy('project_location', 'projects', array(
        'label' => 'Locations',
        'hierarchical' => true,
    ...
    register_taxonomy('project_property', 'projects', array(
        'label' => 'Properties',
        'hierarchical' => true,
...
function handle_project_search() {
    check_ajax_referer('project_search_nonce', 'nonce');
    
    $search_query = sanitize_text_field($_POST['search_query']);
    $results = array();
    
    if (strlen($search_query) >= 2) {
        // Search in custom post type 'projects'
        $args = array(
            'post_type' => 'projects',
            'post_status' => 'publish',
            'posts_per_page' => 10,
...
add_action('wp_ajax_project_search', 'handle_project_search');
add_action('wp_ajax_nopriv_project_search', 'handle_project_search');
...
function display_custom_header_menu() {
    ...
    <div class="sidebar" id="sidebar">
        ...
        <div class="sidebar-menu">
            ...
            <div class="contact-info">
                <h4>Contact Us</h4>
                <p><?php echo esc_html(get_theme_mod('contact_phone_1', '+63 912 333 9999')); ?></p>
                <p><?php echo esc_html(get_theme_mod('contact_phone_2', '+63 999 111 8888')); ?></p>
    ...
    <script>
        ...
        var projectSearchAjax = {
            ajax_url: '<?php echo admin_url('admin-ajax.php'); ?>',
            nonce: '<?php echo wp_create_nonce('project_search_nonce'); ?>'
        };
    </script>
    <?php
}
